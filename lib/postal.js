    ...
    var bindingsResolver = _config.resolver = {
        ...
        purge: function (options) {
            var matchPredicate = function (val, key) {
                ...
                var topic = split[0];
                var binding = split[1];
                if ((!options.topic || options.topic === topic) && (!options.binding || options.binding === binding)) {
                    delete this.cache[key];
                }
            };
            var compactPredicate = function (val, key) {
                var split = key.split("-");
                if (_postal.getSubscribersFor({
                    topic: split[0]
                }).length === 0) {
                    delete this.cache[key];
                }
        ...
        unsubscribe: function unsubscribe() {
            ...
            for (; unSubIdx < unSubLen; unSubIdx++) {
                ...
                if (topicSubs) {
                    ...
                    var autoCompact = _config.autoCompactResolver === true ? 0 : typeof autoCompact === "number" ? (autoCompact - 1) : -1;
